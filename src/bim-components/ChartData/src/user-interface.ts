...
export function ChartUI(components: OBC.Components) {
  ...
  const initialize = () => {
    setTimeout(async () => {
      ...
      const data = await chartClass.getAllEntitiesOfType();
      ...
      canvas.onclick = function (event) {
        ...
        if (activePoints && activePoints.length > 0) {
          const index = activePoints[0].index; // Indeks klikniętego słupka
          const label = chart?.data.labels[index]; // Etykieta klikniętego słupka
          const value = chart?.data.datasets[0].data[index]; // Wartość klikniętego słupka

          const clickedBarData = data.filter(
            (item) => item.elementType === label,
          )[0];
          const models = fragmdntManager.groups;
          for (const [id, model] of models) {
            const fragmentMapOfBarIds = model.getFragmentMap(
              clickedBarData.expressIds,
            );
          ...
          currentColors[index] =
            currentColors[index] === clickedColor
              ? defaultColors[index]
              : clickedColor;
          chart?.update();
        }
    ...
    if (chartType === "bar") {
      ...
      canvas.onclick = function (event) {
        ...
        if (activePoints && activePoints.length > 0) {
          ...
          const clickedBarData = levelsData.filter(
            (item) => item.elementType === label,
          )[0];
          const models = fragmdntManager.groups;
          for (const [modelId, model] of models) {
            const fragmentIdMapFromSelectedChartELement = model.getFragmentMap(
              clickedBarData.expressIds,
            );
