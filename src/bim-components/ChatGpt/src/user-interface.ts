...
export function ModalDataUI(components:OBC.Components){
  ...
  textArea.style.backgroundColor = "rgb(46, 51, 56)";
  ...
  const resetTextInputs = async ()=>{
    textArea.innerText = "";
    const inputQuerry = document.getElementById("querryDataField") as BUI.TextInput;
    if(inputQuerry as BUI.TextInput)
    inputQuerry.value="";
    const fragmentManager = components.get(OBC.FragmentsManager);
    for(const [_, model] of fragmentManager.groups){
      const modelTypeWall = await model.getAllPropertiesOfType(WEBIFC.IFCWALLSTANDARDCASE);
